...
export function PluginOverview({ activeTab, examples, drafts, catalogEntries, installed, onCreate, onTabChange, onDeleteDraft, onActivateDraft = () => undefined, onDeactivateDraft, onEditInstalled = () => undefined, onToggleInstalled = () => undefined, onUninstallInstalled = () => undefined }: PluginOverviewProps) {
  ...
  return <>
    ...
    {activeTab === "allgemein" ? <>
      ...
      <aside className="plugins-skill-note" aria-label="Plugin Creator Skill"><ExtensionsIcon className="h-4 w-4" /><span><strong>Plugin Creator</strong> erstellt denselben persönlichen Draft-Flow direkt mit deinem Coding-Agenten.</span><code>$plugin-creator</code></aside>
    </> : null}
    ...
    {activeTab === "eigene" ? <section className="plugins-section plugins-tab-section" aria-labelledby="own-plugins-title">
      ...
      {drafts.length > 0 ? <div className="plugins-draft-list">{drafts.map((draft) => <article className="plugin-draft-card plugin-draft-row" key={draft.id}>
      ...
      </article>)}</div> : <div className="plugins-empty"><ExtensionsIcon className="h-5 w-5" /><strong>Noch kein eigenes Plugin</strong><span>Starte mit dem empfohlenen KI-Prompt oder erstelle es visuell.</span><button type="button" className="quiet-button-primary" onClick={onCreate}>Ersten Draft anlegen</button></div>}
    </section> : null}
