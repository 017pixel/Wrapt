...
export function PluginAiWizard({ draftId, draft, onChange, onClose, onComplete, purpose = "create" }: PluginAiWizardProps) {
  ...
  const renderGoal = () => <>
    <div className="plugin-panel-heading"><div><span className="plugins-kicker">Schritt 2</span><h3>{editMode ? "Was soll geändert werden?" : "Was soll entstehen?"}</h3></div><PluginHelpPopover title={editMode ? "Änderungen" : "Ziel"}>{editMode ? "Beschreibe zuerst die gewünschte Änderung. Der Agent soll das bestehende Plugin lesen, erhaltene Funktionen bewahren und nur die nötigen Dateien anpassen." : "Je konkreter das Ziel und die Zielgruppe sind, desto besser kann dein KI-Agent eine passende Oberfläche und sinnvolle Aktionen bauen."}</PluginHelpPopover></div>
    <div className="plugin-form-grid">{editMode ? <Field label="Konkrete Änderungswünsche" className="is-wide"><textarea aria-label="Konkrete Änderungswünsche" value={draft.wizard.editRequest} onChange={(event) => updateWizard({ editRequest: event.target.value })} placeholder="Was soll besser, anders oder zusätzlich funktionieren?" /></Field> : null}<Field label="Ziel des Plugins" className="is-wide"><textarea aria-label="Ziel des Plugins" value={draft.wizard.goal} onChange={(event) => updateWizard({ goal: event.target.value })} placeholder="Welche Aufgabe soll das Plugin lösen?" /></Field><Field label="Zielgruppe"><textarea aria-label="Zielgruppe" value={draft.wizard.audience} onChange={(event) => updateWizard({ audience: event.target.value })} placeholder="Wer nutzt es?" /></Field><Field label="Weitere Beschreibung"><textarea aria-label="Weitere Beschreibung" value={draft.wizard.additionalDescription} onChange={(event) => updateWizard({ additionalDescription: event.target.value })} placeholder="Wichtige Details, Beispiele oder Grenzen" /></Field></div>
  </>;
