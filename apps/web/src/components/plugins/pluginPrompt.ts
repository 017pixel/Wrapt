...
function restartInstruction(behavior: PluginDraftContent["wizard"]["restartBehavior"]): string {
  ...
  if (behavior === "approved") return "Starte Frontend oder Backend nur nach einer ausdrücklichen Freigabe des Users neu und erkläre vorher die Auswirkungen.";
  return "Wenn ein Neustart nötig wäre, musst du vorher fragen und auf die Freigabe des Users warten. Starte Frontend oder Backend nicht eigenständig neu.";
}
...
export function buildPluginAgentPrompt(draft: PluginDraftContent, options: PluginPromptOptions = {}): string {
  ...
  return [
    ...
    "Nutze ausschließlich die bestehende Plugin-Aktionsschicht und den Host-Broker für open-route, copy-text, toggle-panel, notify, open-overlay, open-bottom-sheet, set-filter, save-state, load-state, run-command, refresh-data, start-timer, stop-timer und reset-timer. open-route darf nur eine interne Route mit / oder eine validierte HTTP(S)-Fallback-URL öffnen; niemals javascript:, data:, freie Fensterparameter oder fremde Host-DOM-Flächen. Jede Funktion braucht eine sichtbare UI-Aktion, eine deklarierte Aktion und die niedrigste notwendige Permission. Keine stillen Seiteneffekte und keine fremden Host-Flächen direkt manipulieren.",
    ...
    "8. Während dieses Auftrags keinen Frontend- oder Backend-Neustart eigenständig durchführen. " + restartInstruction(wizard.restartBehavior) + " Ein laufendes Terminal oder eine KI-Session darf dadurch nicht unterbrochen werden. Für den User gibt es in Wrapt einen manuellen Neustart-/Neuladen-Weg.",
