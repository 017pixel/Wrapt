...
export const wraptQueries = {
  ...
  restartStatus: () =>
    queryOptions({ queryKey: ["system", "restart-status"], queryFn: ({ signal }) => apiClient.restartStatus(signal), refetchInterval: 25_000, staleTime: 5_000 }),
  codexResetHistory: () =>
  ...
  updateStatus: () =>
    queryOptions({ queryKey: ["system", "update-status"], queryFn: ({ signal }) => apiClient.updateStatus(signal), staleTime: 30_000 }),
  usageMonitoring: () =>
