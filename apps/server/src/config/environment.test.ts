...
describe("Wrapt-Umgebungsvariablen", () => {
  ...
  it("hebt die vorherigen lokalen Produktstände auf den aktuellen Stand", () => {
    expect(canonicalizeWraptEnvironment({ APP_VERSION: "0.95.0" }).APP_VERSION).toBe("1.1.0");
    expect(canonicalizeWraptEnvironment({ APP_VERSION: "0.96.1" }).APP_VERSION).toBe("1.1.0");
    expect(canonicalizeWraptEnvironment({ APP_VERSION: "0.97.0" }).APP_VERSION).toBe("1.1.0");
    expect(canonicalizeWraptEnvironment({ APP_VERSION: "0.98.0" }).APP_VERSION).toBe("1.1.0");
    expect(canonicalizeWraptEnvironment({ APP_VERSION: "0.99.0" }).APP_VERSION).toBe("1.1.0");
    expect(canonicalizeWraptEnvironment({ APP_VERSION: "0.99.5" }).APP_VERSION).toBe("1.1.0");
    expect(canonicalizeWraptEnvironment({ APP_VERSION: "1.0.0" }).APP_VERSION).toBe("1.1.0");
    expect(canonicalizeWraptEnvironment({ APP_VERSION: "1.0.1" }).APP_VERSION).toBe("1.1.0");
    expect(canonicalizeWraptEnvironment({ APP_VERSION: "1.0.2" }).APP_VERSION).toBe("1.1.0");
  });
});
